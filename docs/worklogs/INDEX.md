# Worklog Index

This index publishes retained WP-00 and requirements-cleanup worklogs, plus historical delete manifests that remain part of the documentation trail.

## Worklogs

```{toctree}
:maxdepth: 1

WORKLOG-20260502-wp07-migration-copied-size-fix
WORKLOG-20260502-wp07-validation-test-fix
WORKLOG-20260502-wp07-review-safety-and-quarantine-record-closure
WORKLOG-20260501-codex-safety-guardrails
WORKLOG-20260501-wp07-validation-quarantine
WORKLOG-20260501-wp06-host-path-resolver-fix
WORKLOG-20260501-wp06-watcher-test-path-portability
WORKLOG-20260501-wp06-watcher-implementation
WORKLOG-20260501-wp06-start-readiness
WORKLOG-20260501-wp05-health-test-fixture-fix
WORKLOG-20260501-wp05-cleanup-followup-patch
WORKLOG-20260501-wp05-review-and-cleanup-scope
WORKLOG-20260501-wp05-fastapi-control-plane-routes
WORKLOG-20260501-wp04-event-outbox-redis-streams-dispatcher
WORKLOG-20260501-wp05-prep-fastapi-control-plane-map
WORKLOG-20260501-wp-reference-maps-post-wp05
WORKLOG-20260430-wp00-cleanup-before-wp01
WORKLOG-20260430-wp00-contract-harness-verification
WORKLOG-20260430-wp01-project-skeleton-settings-shared-types
WORKLOG-20260430-wp01-config-error-code-alignment
WORKLOG-20260430-wp01-repo-hygiene-before-wp02
WORKLOG-20260430-wp02-pydantic-models-schema-binding
WORKLOG-20260430-wp03-database-migrations-repository-interfaces
WORKLOG-20260430-wp04-readiness-event-stream-contract-alignment
WORKLOG-20260430-schema-fixture-validation-fix
WORKLOG-20260430-pytest-contract-import-path-fix
WORKLOG-20260430-codex-task-guide-batch-4
WORKLOG-20260429-l2-requirement-decomposition-batch-2
WORKLOG-20260429-batch2-completion-gate
WORKLOG-20260426-contract-hardening-batch-1
BATCH_PROGRESS-20260426-contract-hardening
WORKLOG-20260425-tag-routing-dashboard-presentation
WORKLOG-20260425-requirements-decomposition
WORKLOG-20260425-requirements-cleanup
WORKLOG-20260425-requirements-clarity-and-decisions
WORKLOG-20260425-requirement-reference-decomposition-alignment
WORKLOG-20260425-owning-requirement-decomposition-traceability
WORKLOG-20260425-lifecycle-handoff-requirements-audit
WORKLOG-20260425-idle-folder-health-requirements
WORKLOG-20260425-decision-closure-boundaries-contracts
WORKLOG-20260425-api-requirement-traceability-cleanup
WORKLOG-20260501-wp04-review-wp05-prompt-hardening
```

## Delete manifests

```{toctree}
:maxdepth: 1

WORKLOG-20260501-codex-safety-guardrails
WORKLOG-20260501-wp05-cleanup-followup-patch
DELETE_MANIFEST-20260425-tag-routing-dashboard-presentation
DELETE_MANIFEST-20260425-retire-req018-decomposition-file
DELETE_MANIFEST-20260425-remove-req018-decomposition-gate
DELETE_MANIFEST-20260425-idle-folder-health-requirements
```
